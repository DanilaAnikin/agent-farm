              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 line-clamp-2 text-xs text-(--color-muted) group-hover:text-(--color-fg)">
                    {/* Popisek jen když zpráva říká něco navíc — ne „Orchestrátor nastartoval: Orchestrátor nastartoval." */}
                    {g.count > 1 || g.text === g.label ? null : (
                      <span className="text-(--color-faint)">{g.label}: </span>
                    )}
                    {g.text}
                  </p>
                  {e.pr_url ? (
